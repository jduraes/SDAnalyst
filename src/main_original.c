#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include "pico/stdlib.h"
#include "hardware/spi.h"
#include "hardware/gpio.h"
#include "sd_card.h"

#define VERSION "1.4.0"
#define BUILD_DATE __DATE__
#define BUILD_TIME __TIME__

// SD card SPI pins
#define SD_SPI_PORT spi0
#define SD_PIN_MISO 4
#define SD_PIN_CS   5
#define SD_PIN_SCK  2
#define SD_PIN_MOSI 3

// Structure to hold SD card analysis results
typedef struct {
    sd_card_info_t card_info;
    bool has_mbr;
    bool has_gpt;
    uint32_t partition_count;
} sd_analysis_t;

// Core SD card functions
int sd_card_initialize(void);
int sd_card_get_analysis(sd_analysis_t* analysis);
void sd_card_print_info(const sd_card_info_t* card_info);
void sd_card_show_startup_banner(void);

// Partition analysis functions
int analyze_partition_table(sd_analysis_t* analysis);
void analyze_partition_filesystem(uint32_t start_lba, const char* partition_name);
void analyze_gpt_partitions(void);
void analyze_mbr_partitions(void);

// Filesystem analysis functions
void analyze_fat_filesystem(uint32_t start_lba);
void analyze_exfat_filesystem(uint32_t start_lba);
void analyze_ext_filesystem(uint32_t start_lba);
void list_fat_directory(uint32_t dir_start_lba, const char* path);

// Utility functions
void print_hex_dump(uint8_t *data, size_t len, size_t offset);
void read_and_display_sector(uint32_t sector_num);
void format_fat_datetime(uint16_t date, uint16_t time, char* output, size_t output_size);

void print_hex_dump(uint8_t *data, size_t len, size_t offset) {
    for (size_t i = 0; i < len; i++) {
        if (i % 16 == 0) {
            printf("%08X: ", (unsigned int)(offset + i));
        }
        printf("%02X ", data[i]);
        if (i % 16 == 15 || i == len - 1) {
            // Print ASCII representation
            int padding = 15 - (i % 16);
            for (int p = 0; p < padding; p++) {
                printf("   ");
            }
            printf(" |");
            for (size_t j = i - (i % 16); j <= i; j++) {
                char c = (data[j] >= 32 && data[j] <= 126) ? data[j] : '.';
                printf("%c", c);
            }
            printf("|\n");
        }
    }
}

void read_and_display_sector(uint32_t sector_num) {
    uint8_t buffer[512];
    printf("\n--- Reading sector %u ---\n", sector_num);
    
    if (sd_read_block(sector_num, buffer) == 0) {
        print_hex_dump(buffer, 512, sector_num * 512);
    } else {
        printf("Error reading sector %u\n", sector_num);
    }
}

void analyze_partition_filesystem(uint32_t start_lba, const char* partition_name) {
    uint8_t boot_sector[512];
    printf("  Analyzing filesystem...\n");
    
    if (sd_read_block(start_lba, boot_sector) != 0) {
        printf("  Error reading boot sector\n");
        return;
    }
    
    // Check for FAT filesystem signatures
    if ((boot_sector[510] == 0x55 && boot_sector[511] == 0xAA) && 
        (memcmp(&boot_sector[54], "FAT12   ", 8) == 0 || 
         memcmp(&boot_sector[54], "FAT16   ", 8) == 0 || 
         memcmp(&boot_sector[82], "FAT32   ", 8) == 0)) {
        
        printf("  Filesystem: FAT\n");
        analyze_fat_filesystem(start_lba);
        
    } else if (memcmp(&boot_sector[3], "EXFAT   ", 8) == 0) {
        printf("  Filesystem: exFAT\n");
        analyze_exfat_filesystem(start_lba);
        
    } else if (boot_sector[56] == 0x53 && boot_sector[57] == 0xEF) { // ext2/3/4 magic
        printf("  Filesystem: ext2/3/4\n");
        analyze_ext_filesystem(start_lba);
        
    } else {
        printf("  Filesystem: Unknown or unsupported\n");
        // Show first few bytes to help identify
        printf("  Boot sector signature: ");
        for (int i = 0; i < 16; i++) {
            printf("%02X ", boot_sector[i]);
        }
        printf("\n");
    }
}

void analyze_fat_filesystem(uint32_t start_lba) {
    uint8_t boot_sector[512];
    if (sd_read_block(start_lba, boot_sector) != 0) {
        printf("  Error reading FAT boot sector\n");
        return;
    }
    
    // Parse FAT boot sector
    uint16_t bytes_per_sector = boot_sector[11] | (boot_sector[12] << 8);
    uint8_t sectors_per_cluster = boot_sector[13];
    uint16_t reserved_sectors = boot_sector[14] | (boot_sector[15] << 8);
    uint8_t num_fats = boot_sector[16];
    uint16_t root_entries = boot_sector[17] | (boot_sector[18] << 8);
    uint32_t sectors_per_fat = boot_sector[22] | (boot_sector[23] << 8);
    
    // For FAT32, sectors per FAT is at offset 36
    if (sectors_per_fat == 0) {
        sectors_per_fat = boot_sector[36] | (boot_sector[37] << 8) | 
                         (boot_sector[38] << 16) | (boot_sector[39] << 24);
    }
    
    printf("  Bytes per sector: %u\n", bytes_per_sector);
    printf("  Sectors per cluster: %u\n", sectors_per_cluster);
    printf("  Reserved sectors: %u\n", reserved_sectors);
    printf("  Number of FATs: %u\n", num_fats);
    printf("  Root entries: %u\n", root_entries);
    printf("  Sectors per FAT: %u\n", sectors_per_fat);
    
    // Calculate root directory location
    uint32_t fat_start = start_lba + reserved_sectors;
    uint32_t root_dir_start = fat_start + (num_fats * sectors_per_fat);
    
    printf("  FAT starts at LBA: %u\n", fat_start);
    printf("  Root directory at LBA: %u\n", root_dir_start);
    
    // List files in root directory
    list_fat_directory(root_dir_start, "/");
}

void analyze_exfat_filesystem(uint32_t start_lba) {
    printf("  exFAT filesystem detected but not implemented yet\n");
}

void analyze_ext_filesystem(uint32_t start_lba) {
    printf("  ext2/3/4 filesystem detected but not implemented yet\n");
}

// Helper function to format FAT date/time
void format_fat_datetime(uint16_t date, uint16_t time, char* output, size_t output_size) {
    // FAT date: bits 15-9 = year (from 1980), 8-5 = month, 4-0 = day
    // FAT time: bits 15-11 = hour, 10-5 = minute, 4-0 = second/2
    
    int year = ((date >> 9) & 0x7F) + 1980;
    int month = (date >> 5) & 0x0F;
    int day = date & 0x1F;
    int hour = (time >> 11) & 0x1F;
    int minute = (time >> 5) & 0x3F;
    
    // Format like "Aug 11 20:07" (Unix ls style)
    const char* months[] = {"???", "Jan", "Feb", "Mar", "Apr", "May", "Jun",
                           "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"};
    
    if (month >= 1 && month <= 12 && day >= 1 && day <= 31) {
        snprintf(output, output_size, "%s %2d %02d:%02d", 
                months[month], day, hour, minute);
    } else {
        snprintf(output, output_size, "??? ?? ??:??");
    }
}

void list_fat_directory(uint32_t dir_start_lba, const char* path) {
    uint8_t buffer[512];
    printf("\n  === Directory listing for %s ===\n", path);
    
    if (sd_read_block(dir_start_lba, buffer) != 0) {
        printf("  Error reading directory sector\n");
        return;
    }
    
    // Parse directory entries (32 bytes each)
    int file_count = 0;
    uint64_t total_size = 0;
    char long_filename[256] = {0};  // Buffer for long filename
    
    for (int i = 0; i < 16; i++) {
        uint8_t *entry = &buffer[i * 32];
        
        // Check if entry is empty
        if (entry[0] == 0x00) break;
        if (entry[0] == 0xE5) continue; // Deleted file
        
        // Check for Long Filename (LFN) entry
        if (entry[11] == 0x0F) {
            // This is a Long Filename entry
            uint8_t sequence = entry[0] & 0x3F;  // Sequence number (without last entry flag)
            bool is_last = (entry[0] & 0x40) != 0;  // Last LFN entry flag
            
            // Calculate position in long filename buffer
            int pos = (sequence - 1) * 13;
            
            if (pos >= 0 && pos < 243) {  // Ensure we don't overflow
                // Extract Unicode characters from LFN entry
                // Characters 1-5: bytes 1-10 (UTF-16)
                for (int j = 0; j < 5; j++) {
                    uint16_t unicode_char = entry[1 + j * 2] | (entry[2 + j * 2] << 8);
                    if (unicode_char == 0 || unicode_char == 0xFFFF) break;
                    if (pos + j < 255) long_filename[pos + j] = (unicode_char < 128) ? (char)unicode_char : '?';
                }
                
                // Characters 6-11: bytes 14-25 (UTF-16)
                for (int j = 0; j < 6; j++) {
                    uint16_t unicode_char = entry[14 + j * 2] | (entry[15 + j * 2] << 8);
                    if (unicode_char == 0 || unicode_char == 0xFFFF) break;
                    if (pos + 5 + j < 255) long_filename[pos + 5 + j] = (unicode_char < 128) ? (char)unicode_char : '?';
                }
                
                // Characters 12-13: bytes 28-31 (UTF-16)
                for (int j = 0; j < 2; j++) {
                    uint16_t unicode_char = entry[28 + j * 2] | (entry[29 + j * 2] << 8);
                    if (unicode_char == 0 || unicode_char == 0xFFFF) break;
                    if (pos + 11 + j < 255) long_filename[pos + 11 + j] = (unicode_char < 128) ? (char)unicode_char : '?';
                }
            }
            continue; // Skip to next entry
        }
        
        // This is a regular 8.3 directory entry
        
        // Extract short filename as backup
        char short_filename[13] = {0};
        memcpy(short_filename, entry, 8);
        short_filename[8] = 0;
        
        // Trim spaces from short filename
        for (int j = 7; j >= 0 && short_filename[j] == ' '; j--) {
            short_filename[j] = 0;
        }
        
        // Add extension if present
        if (entry[8] != ' ') {
            if (strlen(short_filename) > 0) strcat(short_filename, ".");
            strncat(short_filename, (char*)&entry[8], 3);
            // Trim spaces from extension
            int len = strlen(short_filename);
            for (int j = len - 1; j >= 0 && short_filename[j] == ' '; j--) {
                short_filename[j] = 0;
            }
        }
        
        uint8_t attributes = entry[11];
        uint32_t file_size = entry[28] | (entry[29] << 8) | 
                            (entry[30] << 16) | (entry[31] << 24);
        
        // Extract date and time (modified time)
        uint16_t mod_time = entry[22] | (entry[23] << 8);
        uint16_t mod_date = entry[24] | (entry[25] << 8);
        
        // Determine which filename to display
        const char* display_name = (long_filename[0] != 0) ? long_filename : 
                                  (short_filename[0] != 0) ? short_filename : "<no name>";
        
        // Format like Unix 'ls -l'
        printf("  ");
        
        // File type and permissions
        printf("%c", (attributes & 0x10) ? 'd' : '-');  // directory
        
        // Owner permissions
        printf("r");  // Always readable in FAT
        printf("%c", (attributes & 0x01) ? '-' : 'w');  // read-only flag inverted
        printf("%c", (attributes & 0x10) ? 'x' : '-');  // directories are executable
        
        // Group permissions (same as owner for FAT)
        printf("r");
        printf("%c", (attributes & 0x01) ? '-' : 'w');
        printf("%c", (attributes & 0x10) ? 'x' : '-');
        
        // Other permissions (same as owner for FAT)
        printf("r");
        printf("%c", (attributes & 0x01) ? '-' : 'w');
        printf("%c", (attributes & 0x10) ? 'x' : '-');
        
        // Number of links (always 1 for FAT)
        printf(" %2d", 1);
        
        // File size (right-aligned with fixed width for alignment)
        if (attributes & 0x10) {
            printf(" %10s", "<DIR>");
        } else {
            printf(" %10u", file_size);
            total_size += file_size;
        }
        
        // Date and time
        char datetime_str[32];
        format_fat_datetime(mod_date, mod_time, datetime_str, sizeof(datetime_str));
        printf(" %s", datetime_str);
        
        // Filename
        printf(" %s", display_name);
        
        // Show 8.3 name in square brackets if it differs from LFN
        if (long_filename[0] != 0 && strcmp(long_filename, short_filename) != 0) {
            printf(" [%s]", short_filename);
        }
        
        printf("\n");
        file_count++;
        
        // Clear long filename buffer for next file
        memset(long_filename, 0, sizeof(long_filename));
    }
    
    // Print summary like 'ls -l'
    printf("  total %d\n", (int)(total_size / 1024));  // Total in KB
    printf("  %d files and directories\n", file_count);
}

void analyze_gpt() {
    uint8_t gpt_header[512];
    printf("\n=== GPT Partition Table Analysis ===\n");
    
    // Read GPT header (sector 1)
    if (sd_read_block(1, gpt_header) != 0) {
        printf("Error reading GPT header\n");
        return;
    }
    
    // Check GPT signature "EFI PART"
    if (memcmp(gpt_header, "EFI PART", 8) != 0) {
        printf("Invalid GPT signature\n");
        return;
    }
    
    printf("Valid GPT signature found\n");
    
    // Parse GPT header
    uint32_t partition_entry_lba = gpt_header[72] | (gpt_header[73] << 8) | 
                                  (gpt_header[74] << 16) | (gpt_header[75] << 24);
    uint32_t num_partitions = gpt_header[80] | (gpt_header[81] << 8) | 
                             (gpt_header[82] << 16) | (gpt_header[83] << 24);
    uint32_t entry_size = gpt_header[84] | (gpt_header[85] << 8) | 
                         (gpt_header[86] << 16) | (gpt_header[87] << 24);
    
    printf("Number of partitions: %u\n", num_partitions);
    printf("Partition entry size: %u bytes\n", entry_size);
    printf("Partition entries start at LBA: %u\n", partition_entry_lba);
    
    // Read partition entries (typically at sector 2)
    uint8_t partition_table[512];
    if (sd_read_block(partition_entry_lba, partition_table) != 0) {
        printf("Error reading GPT partition entries\n");
        return;
    }
    
    printf("\nGPT Partitions:\n");
    
    // Parse up to 4 partitions (128 bytes each in a 512-byte sector)
    uint32_t partitions_per_sector = 512 / entry_size;
    uint32_t partitions_to_check = (num_partitions < partitions_per_sector) ? 
                                   num_partitions : partitions_per_sector;
    
    for (uint32_t i = 0; i < partitions_to_check; i++) {
        uint8_t *entry = &partition_table[i * entry_size];
        
        // Check if partition exists (non-zero type GUID)
        bool empty = true;
        for (int j = 0; j < 16; j++) {
            if (entry[j] != 0) {
                empty = false;
                break;
            }
        }
        
        if (empty) continue;
        
        // Get partition start and end LBAs
        uint64_t start_lba = 0, end_lba = 0;
        for (int j = 0; j < 8; j++) {
            start_lba |= ((uint64_t)entry[32 + j]) << (j * 8);
            end_lba |= ((uint64_t)entry[40 + j]) << (j * 8);
        }
        
        // Extract partition name (UTF-16, so take every other byte)
        char name[37] = {0};
        for (int j = 0, k = 0; j < 72 && k < 36; j += 2, k++) {
            name[k] = entry[56 + j];
            if (name[k] == 0) break;
        }
        
        printf("\nPartition %u:\n", i + 1);
        printf("  Name: %s\n", name[0] ? name : "(unnamed)");
        printf("  Start LBA: %llu\n", start_lba);
        printf("  End LBA: %llu\n", end_lba);
        printf("  Size: %llu sectors (%.2f MB)\n", 
               end_lba - start_lba + 1, 
               ((end_lba - start_lba + 1) * 512.0) / (1024 * 1024));
        
        // Try to identify filesystem type
        analyze_partition_filesystem((uint32_t)start_lba, name);
    }
}

void analyze_mbr() {
    uint8_t mbr[512];
    printf("\n=== Master Boot Record Analysis ===\n");
    
    if (sd_read_block(0, mbr) != 0) {
        printf("Error reading MBR\n");
        return;
    }
    
    // Check boot signature
    if (mbr[510] != 0x55 || mbr[511] != 0xAA) {
        printf("Invalid boot signature: %02X %02X (expected 55 AA)\n", mbr[510], mbr[511]);
        return;
    }
    
    printf("Valid boot signature found\n");
    printf("Parsing partition table...\n");
    
    // Parse partition table
    for (int i = 0; i < 4; i++) {
        printf("Processing partition %d...\n", i + 1);
        uint8_t *partition = &mbr[446 + i * 16];
        printf("  Reading partition data...\n");
        
        uint8_t status = partition[0];
        printf("  Status read: 0x%02X\n", status);
        
        uint8_t type = partition[4];
        printf("  Type read: 0x%02X\n", type);
        
        // Read LBA values carefully to avoid potential alignment issues
        uint32_t lba_start = partition[8] | (partition[9] << 8) | (partition[10] << 16) | (partition[11] << 24);
        printf("  LBA start read: %u\n", lba_start);
        
        uint32_t lba_size = partition[12] | (partition[13] << 8) | (partition[14] << 16) | (partition[15] << 24);
        printf("  LBA size read: %u\n", lba_size);
        
        printf("Partition %d:\n", i + 1);
        printf("  Status: 0x%02X (%s)\n", status, (status == 0x80) ? "Bootable" : "Not bootable");
        printf("  Type: 0x%02X", type);
        switch (type) {
            case 0x00: printf(" (Empty)"); break;
            case 0x01: printf(" (FAT12)"); break;
            case 0x04: printf(" (FAT16 <32MB)"); break;
            case 0x06: printf(" (FAT16)"); break;
            case 0x0B: printf(" (FAT32)"); break;
            case 0x0C: printf(" (FAT32 LBA)"); break;
            case 0x0E: printf(" (FAT16 LBA)"); break;
            case 0x83: printf(" (Linux)"); break;
            case 0xEE: printf(" (GPT Protective MBR)"); break;
            default: printf(" (Unknown)"); break;
        }
        printf("\n");
        printf("  LBA Start: %u\n", lba_start);
        printf("  Size (sectors): %u\n", lba_size);
        printf("  Size (MB): %.2f\n", (lba_size * 512.0) / (1024 * 1024));
        printf("\n");
        
        // Check for GPT protective MBR
        if (type == 0xEE) {
            printf("GPT protective MBR detected - analyzing GPT...\n");
            analyze_gpt();
            return; // GPT analysis covers all partitions
        }
        
        // If this is a regular partition, analyze its filesystem
        if (lba_start > 0 && lba_size > 0) {
            char partition_name[16];
            snprintf(partition_name, sizeof(partition_name), "Partition %d", i + 1);
            analyze_partition_filesystem(lba_start, partition_name);
        }
    }
}


int main() {
    stdio_init_all();
    
    // Wait a moment for USB serial to be ready
    sleep_ms(2000);
    
    printf("********************************************************************************\n");
    printf("Raspberry Pi Pico SD Card Reader\n");
    printf("Version: %s\n", VERSION);
    printf("Built: %s %s\n", BUILD_DATE, BUILD_TIME);
    printf("================================\n");
    
    // Initialize SD card
    printf("Initializing SD card...\n");
    printf("SPI pins: SCK=%d, MOSI=%d, MISO=%d, CS=%d\n", SD_PIN_SCK, SD_PIN_MOSI, SD_PIN_MISO, SD_PIN_CS);
    
    int init_result = sd_init(SD_SPI_PORT, SD_PIN_SCK, SD_PIN_MOSI, SD_PIN_MISO, SD_PIN_CS);
    if (init_result != 0) {
        printf("Failed to initialize SD card! Error code: %d\n", init_result);
        printf("\nTroubleshooting:\n");
        printf("1. Check all SPI connections are secure\n");
        printf("2. Ensure SD card is properly inserted\n");
        printf("3. Try a different SD card\n");
        printf("4. Check power supply (3.3V for SD card)\n");
        printf("5. Verify pin connections match the code\n");
        while (1) sleep_ms(1000);
    }
    
    printf("SD card initialized successfully!\n");
    
    // Get card info
    sd_card_info_t card_info;
    if (sd_get_info(&card_info) == 0) {
        printf("\nSD Card Information:\n");
        printf("Type: %s\n", card_info.type == SD_CARD_TYPE_SD1 ? "SD1" : 
                           card_info.type == SD_CARD_TYPE_SD2 ? "SD2" : 
                           card_info.type == SD_CARD_TYPE_SDHC ? "SDHC" : "Unknown");
        printf("Capacity: %.2f MB (%u blocks)\n", 
               (card_info.blocks * 512.0) / (1024 * 1024), 
               card_info.blocks);
        printf("Block size: %u bytes\n", card_info.block_size);
    }
    
    // Analyze MBR
    analyze_mbr();
    
    // Read and display first few sectors
    printf("\n=== Raw sector data ===\n");
    read_and_display_sector(0);  // MBR
    
    printf("\nSD card analysis complete!\n");
    printf("Connect to USB serial to see output.\n");
    
    // Keep the program running
    while (1) {
        sleep_ms(1000);
    }
    
    return 0;
}
